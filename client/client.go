...
// NewAuto retrieves the config from the supplied single server and uses that config to create a new StoreClient
// that can talk to the entire cluster.
func NewAuto(hostport string) (*StoreClient, error) {
	// Create a client with a temporary connection to a single server
	client := NewDirect(hostport)
